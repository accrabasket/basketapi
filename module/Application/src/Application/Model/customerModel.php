...
class customerModel  {
    ...
    function assignedOrderToRider($where, $optional = array()) {
        try {
            ...
            $query = $query->join('order_master', 'order_master.order_id = order_assignments.order_id',array('store_id','shipping_address_id', 'order_status', 'payable_amount','user_id','merchant_id','updated_date'));
            if(!empty($optional['columns'])) {
                $query->columns($optional['columns']);
            }
            if(!empty($where['order_id'])) {
